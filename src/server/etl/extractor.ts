...
export async function extractFeed(source: DataSource, retries = 3): Promise<ExtractResult> {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if ((err as any).isTerminal) {
        console.error(`Terminal error encountered for ${source.name}. Aborting retries.`);
        throw new Error(`Terminal error extracting feed ${source.name}: HTTP ${lastStatus} ${lastStatusText} - ${err.message}`, { cause: error });
      }
      
      if (attempt === retries) {
        throw new Error(`Failed to extract feed ${source.name} after ${retries} attempts: HTTP ${lastStatus} ${lastStatusText} - ${err.message}`, { cause: error });
      }
      
      // Wait before retrying
      const delay = (err as any).retryDelay || (Math.pow(2, attempt) * 1000 + Math.floor(Math.random() * 1000));
