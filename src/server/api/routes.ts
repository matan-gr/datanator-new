      ...
      for await (const line of rl) {
        ...
        try {
          ...
          const appLog = {
            ...
            level: parsed.appLevel || parsed.severity || parsed.level?.toUpperCase() || 'INFO',
...
// Get OAuth URL for GCS
apiRouter.get('/gcs/auth-url', (req, res) => {
  const protocol = req.headers['x-forwarded-proto'] || req.protocol;
  const host = req.headers['x-forwarded-host'] || req.get('host');
  const redirectUri = `${protocol}://${host}/api/v1/gcs/callback`;

  if (!process.env.GOOGLE_CLIENT_ID) {
    // Mock OAuth flow for preview environment
    const mockUrl = `${redirectUri}?code=mock_auth_code_12345`;
    return res.json({ success: true, url: mockUrl });
  }
...
// OAuth Callback for GCS
apiRouter.get('/gcs/callback', (req, res) => {
  ...
  res.send(`
    <html>
      <body>
        <script>
          if (window.opener) {
            window.opener.postMessage({ type: 'GCS_OAUTH_SUCCESS', code: '${code}' }, '*');
            window.close();
          } else {
  ...
  try {
    ...
    if (authCode && !accessToken) {
      if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
        // Mock export for preview environment
        if (authCode === 'mock_auth_code_12345') {
          return res.json({ 
            success: true, 
            message: `[MOCK EXPORT] Successfully exported ${files.length} files to gs://${bucketName}/gcp-datanator-export/` 
          });
        }
        return res.status(500).json({ success: false, error: 'Server is missing Google OAuth credentials (GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET)' });
      }
      ...
      try {
        ...
        return res.status(400).json({ success: false, error: `Failed to exchange auth code: ${error.message}` });
      }
    }
