...
You have two options for deployment: using Cloud Build (automated) or manual Docker build.

### Option A: Automated Deployment (Cloud Build)
...
If you prefer to build the Docker image manually on your local machine or a CI/CD pipeline:
...
docker build -t us-central1-docker.pkg.dev/${PROJECT_ID}/datanator-repo/gcp-datanator:latest -f Dockerfile.txt .

# 3. Push the image to Artifact Registry
docker push us-central1-docker.pkg.dev/${PROJECT_ID}/datanator-repo/gcp-datanator:latest
